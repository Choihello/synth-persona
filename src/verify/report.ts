...
function renderResult(result: StudyResult): string {
  ...
  lines.push("");
  lines.push(
    `- 전체: ${dot(result.signal)} **${result.signal}** (분산 ${result.dispersion.toFixed(2)})`,
  );
  lines.push("");
  ...
  const split = Object.entries(result.bySegment).flatMap(([dim, segs]) =>
    Object.entries(segs)
      .filter(([, s]) => s.signal === "split")
      .map(([val]) => `${dim}=${val}`),
  );
  if (split.length) {
    lines.push(
      `> 🔴 **여기 조사 권장:** ${split.join(", ")} — 의견이 갈리므로 진짜 조사를 조준하세요.`,
    );
    lines.push("");
  }
  return lines.join("\n");
...
function renderCalibration(cal: CalibrationReport): string {
  ...
  if (cal.cases.length) {
    ...
    for (const c of cal.cases) {
      lines.push(
        `| ${c.id} | ${c.rankCorrelation.toFixed(2)} | ${c.shareMAE.toFixed(2)} | ${c.directionHit ? "✅" : "❌"} |`,
      );
    }
    lines.push("");
  }
