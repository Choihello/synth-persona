...
- [x] **창업자 인사이트 리포트 (Plan 4, heuristic v1)** — 진단을 창업자 행동으로 번역: 기회/저항 세그먼트 랭킹(+판단 보류) · 4층 신뢰도 카드 · heuristic 처방(인터뷰 대상/질문·설문·랜딩·7일 플랜, 전부 "규칙 기반으로 파생된 초안입니다(LLM 아님)" 라벨) · markdown 렌더 (`npm run report:demo`, 키 불필요). 이 mock 경로의 처방은 전부 heuristic이다 — LLM 생성은 아래 B3에서 **일부만** 적용된다(drivers/objections·인터뷰 질문).
